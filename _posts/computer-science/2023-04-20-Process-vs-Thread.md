...
- 본 내용은 [쉬운코드 유튜브 영상](https://www.youtube.com/@ez.)에 내용을 조금 더 추가했습니다. 레퍼런스에 각 부분마다 링크가 달려있으니 참고 하시길 바랍니다.
...
<p>
    <img src="/assets/images/post/cs/process-thread/multi-programming.png" width="400" height="300" class="projects__article__img__center">
    <p align="center">
...
자, 그래서 프로세스의 아쉬운 점을 해결하기 위해 나온 방법이, **스레드(Thread)**, 아래와 같은 특징을 가지죠.
...
가만 보면 스레드는 프로세스를 작은 단위로 더 쪼개놓은 것 같지 않으신가요? 그런 스레드에 종류로는 **하드웨어 스레드, OS 스레드, 네이티브 스레드, 커널 스레드, 유저 스레드, 그린 스레드** 가 있습니다. 스레드의 컨셉을 하드웨어, OS, 어플리케이션으로 나눈 듯 보이네요.
...
그럼 이제 멀티 프로세스, 멀티 스레드, 그리고 멀티 코어를 각각 고려해 볼 수 있겠죠?
...
- **IO bound, CPU bound**에서 몇 개의 멀티스레드를 하면 좋을까?
...
- 그럼 언제 Thread를 써야돼? I/O bound 작업이 많아 Thread가 대기해야하는 경우!
- [PyCon US 2021에서 발표한 GIL 메인 컨트리뷰터 나동희님](https://www.youtube.com/watch?v=V18ceQO_JkM)은 “Python이나 Java는 자동으로 메모리 관리를 하는데, Python의 경우 **Reference Counting 방법이 GIL**을 필요로하기 때문에 GIL자체가 사라지는 것은 아직은 아니다.”고 언급하셨습니다.

- Reference Counting

    파이썬에서 메모리를 관리하는 방식으로, 파이썬의 모든 객체를 카운트하며 객체가 참조될 떄 증가, 참조가 삭제되면 감소하는 방식으로 동작합니다. 이 카운트가 0이 되면 삭제 대상이 되며 삭제 cycle이 되면 메모리 할당이 해제하는 방식을 말합니다.


여기까지 프로세스, 스레드 그리고 코어에 대해서 알아봤습니다. 다음번에는 멀티 프로세스, 스레드시에 생기는 Race Condition과 그에 대한 종류, 해결방안, 그리고 CPU 스케줄링에 대해 다뤄보도록 하겠습니다.
